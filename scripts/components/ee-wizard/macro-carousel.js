...
function getEvtListenerOptions(isPassive) {
  return _passiveEvtListenersSupported() ? {
    passive: isPassive
  } : false;
}
...
function setCSSCustomProperty(element, propertyName, propertyValue) {
  ...
  if (window.ShadyCSS) {
    window.ShadyCSS.styleSubtree(element, {
...
value. Add CSS units to its value to avoid breaking the slides layout.`);
  ...
  _updateNavigation() {
    ...
    if (this.navigation) {
      ...
      // update `disabled`
      this._prevButton.disabled = !this.loop && this.selected === 0;
      this._nextButton.disabled = !this.loop && this.selected === this._lastViewIndex;
  ...
  _getSlides() {
    ...
    // to not be picked up)
    this._slidesSlot.assignedNodes({
      flatten: true
    }).forEach(node => {
    ...
    const slideElements = this._slidesSlot.assignedNodes({
        flatten: true
      })
